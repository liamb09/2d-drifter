...
Car::Car (const float &initSCALE) : SCALE(initSCALE), x(0), y(0), vx(0), vy(0), st_angle(0), ACC(0.05), theta(0), TURN_SPEED(0.05), FRICTION(0.95), LENGTH(4.21*initSCALE), WIDTH(1.6*initSCALE), WHEEL_BASE(2.83*initSCALE), TIRE_WIDTH(0.6*initSCALE), TIRE_ANGLE_MAX(0.6), STEERING_WHEEL_MAX(3*M_PI), STEERING_WHEEL_RETURN(0.95), BODY_COLOR({255, 0, 0, 255}) {}
Car::Car (float initX, float initY, const float &initSCALE, const SDL_Color initBODY_COLOR) : SCALE(initSCALE), x(initX), y(initY), vx(0), vy(0), ACC(0.05), st_angle(0), theta(0), TURN_SPEED(0.05), FRICTION(0.95), LENGTH(4.21*initSCALE), WIDTH(1.6*initSCALE), WHEEL_BASE(2.83*initSCALE), TIRE_WIDTH(0.6*initSCALE), TIRE_ANGLE_MAX(0.6), STEERING_WHEEL_MAX(3*M_PI), STEERING_WHEEL_RETURN(0.99), BODY_COLOR(initBODY_COLOR) {}
Car::Car (float initX, float initY, float initACC, float initTURN_SPEED, float initFRICTION, float initLENGTH, float initWIDTH, float initWHEEL_BASE, float initTIRE_WIDTH, float initTIRE_ANGLE_MAX, float initSTEERING_WHEEL_MAX, float initSTEERING_WHEEL_RETURN, const float &initSCALE, const SDL_Color initBODY_COLOR) : SCALE(initSCALE), x(initX), y(initY), vx(0), vy(0), ACC(initACC), st_angle(0), theta(0), TURN_SPEED(initTURN_SPEED), FRICTION(initFRICTION), WIDTH(initWIDTH*initSCALE), LENGTH(initLENGTH*initSCALE), WHEEL_BASE(initWHEEL_BASE*initSCALE), TIRE_WIDTH(initTIRE_WIDTH*initSCALE), TIRE_ANGLE_MAX(initTIRE_ANGLE_MAX), STEERING_WHEEL_MAX(initSTEERING_WHEEL_MAX), STEERING_WHEEL_RETURN(initSTEERING_WHEEL_RETURN), BODY_COLOR(initBODY_COLOR) {}
...
vector<vector<SDL_Vertex>> Car::getVertices () {
    ...
    vector<vector<SDL_Vertex>> result = {
        // Body
        {vtx(LENGTH/2, WIDTH/2, BODY_COLOR), vtx(-LENGTH/2, WIDTH/2, BODY_COLOR), vtx(-LENGTH/2, -WIDTH/2, BODY_COLOR)},
        {vtx(LENGTH/2, WIDTH/2, BODY_COLOR), vtx(LENGTH/2, -WIDTH/2, BODY_COLOR), vtx(-LENGTH/2, -WIDTH/2, BODY_COLOR)},
        // Front right wheel
...
float Car::getSteeringAngle () {
    return st_angle/STEERING_WHEEL_MAX * (M_PI_2-0.6);
}
float Car::getTurnRadius () {
    return WHEEL_BASE/sin(getSteeringAngle()) * (TIRE_WIDTH/2);
}
